...
struct gps_gga_t {
    ...
    bool north; // true if North, false if South
    bool east;  // true if East, false if West
} gga;
struct gps_gll_t {
    float lat;
    float lon;
...
gps_error_code_t parse_gga(gps_t gps_instance, const char* sentence, int len) {
    ...
    for (int i = 0, j = 0; i < len; i = j+1) {
        ...
        if (fieldc == 1) {
            ...
            // Geoid Separation Metres
            if (!empty_field) {
                gga.geoid_sep_metres = strtod(sentence+i, NULL);
            }
            else {
                gga.geoid_sep_metres = 0;  // Important to handle the case where field is empty
            }
        }
